...
function fetch_user_chat_history($from_user_id, $to_user_id, $connect)
{
	...
	foreach($result as $row)
	{
		...
		$chat_message = '';
		$pobierz = '';
		if($row["from_user_id"] == $from_user_id)  /// gdy to wiadomosc uzytkownika
		{
			if($row["status"] == '2')
			{
			...
			{ ////// tu trzeba if ///////
				...
				//$pobierz = '<button type="button" class="btn btn-danger btn-xs dow" id="'.$row['chat_message_id'].'">d</button>';
			}
			

			$dynamic_background = 'background-color:#ffe6e6;';
		}
		else // gdzy to nie jest jego wiadomosc 
		{
			if($row["status"] == '2')
			{
				$chat_message = '<em>This message has been removed</em>';
...
function count_unseen_message($from_user_id, $to_user_id, $connect)// powiadomienie o wiadomosci
{
	$query = "
	SELECT * FROM chat_message 
	WHERE from_user_id = '$from_user_id' 
